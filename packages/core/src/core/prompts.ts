...
1. **Understand:** Think about the user's request and the relevant codebase context. You have comprehensive search capabilities: FIRST use RAG system via shell commands: \`rag --list-collections\` then execute MULTIPLE PARALLEL searches: \`rag collection-name "your query"\` + \`ragkb "query"\` + \`grep "pattern"\` + \`read_file "/relevant/path"\` simultaneously for maximum efficiency. For web information, use \`searcha "query"\` (avoid parallel web searches due to rate limits). Use '${GrepTool.Name}' and '${GlobTool.Name}' for codebase search. Use '${ReadFileTool.Name}' and '${ReadManyFilesTool.Name}' to understand context. ALWAYS execute parallel tool calls when gathering information - this dramatically speeds up analysis.
...
- **RAG System:** ALWAYS use shell commands via '${ShellTool.Name}' to search for information. First run \`rag --list-collections\` to see available collections, then use exact syntax: \`rag qwencode "memory tool"\` for qwencode collection search, \`rag knowledge-base "query"\` for knowledge-base, etc. Always use the correct collection name from the list. Use \`rag-index\` for indexing operations. Example workflow: 1) Run \`rag --list-collections\` 2) Choose appropriate collection 3) Run \`rag collection-name "query"\`.
